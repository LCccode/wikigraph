...
If `ingest-verify` reports FAILED, surface every error line **verbatim** to the user. The only legitimate fix is to re-run `ingest-run` after placing files in `raw/inbox/`. For OpenClaw default KB, use `~/.openclaw/lcwiki/`.
...
**OpenClaw platform — subagent dispatch (same protocol, smaller chunks)**

- N ≤ 3 files: main agent processes sequentially itself.
- N > 3 files: **must** dispatch to subagents. Chunk size = 5-8 files per subagent.
- Dispatch mode: try parallel (all subagent calls in one reply), fall back to sequential if the platform serialises.
- Each subagent uses the same prompt as Claude Code above, with the same hard rules: must call `lcwiki compile-write` per task, never use Write tool to bypass.
- Never have the main agent process all N files in its own context — it causes context degradation / regex-fallback behaviour.

After all subagents finish, main agent runs `lcwiki compile-verify --kb KB` and reports results.
...
Platform-specific dispatch: Claude Code and OpenClaw both use subagent dispatch; only chunk size differs.
...
- **OpenClaw**: chunk size = 10-15 files per subagent, try parallel (all subagent calls in one message) with sequential fallback. Each subagent writes `vault/graph/.extract_chunk_<ID>.json` using the same schema. Main agent merges chunks into `/tmp/lcwiki-extraction.json` before calling `graph-run --extraction`.

Merge command (after all subagents finish, both platforms):
```bash
/opt/anaconda3/bin/python3 -c "
import json, glob
from pathlib import Path
chunks = sorted(Path('KB_PATH/vault/graph').glob('.extract_chunk_*.json'))
...
Do NOT emit one giant extraction JSON in a single LLM turn for > 5 files.
...
Show graph stats and suggest the most interesting question for `/lcwiki query`.
